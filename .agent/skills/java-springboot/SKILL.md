...
- **Package Structure:** Organize code by (e.g., `com.example.app.controller`, `com.example.app.service`).
...
- **DTOs (Data Transfer Objects):** Use DTOs to expose and consume data in the API layer. Do not expose JPA entities directly to the client. Prefer using Java 17 `record` classes for DTOs and Kafka payloads to keep them immutable and avoid boilerplate code.
...
## Lombok & Boilerplate Policy

- **No Lombok:** Do NOT use Lombok dependencies or Lombok annotations (`@Data`, `@Getter`, `@Setter`, `@Builder`, `@NoArgsConstructor`, `@AllArgsConstructor`, etc.) anywhere in the codebase.
- **Java Records:** Leverage Java 17 `record` classes for DTOs, Kafka payloads, and other immutable structures to minimize boilerplate code cleanly and natively.
- **JPA Entities:** Write standard Java getters, setters, constructors, `equals()`, `hashCode()`, and `toString()` methods manually or generate them via the IDE for mutable JPA entities.
