...
public class OutboxPublisherScheduler {
    ...
    public OutboxPublisherScheduler(
            ...
            @Value("${orders.outbox.publisher.batch-size:50}") int batchSize,
            @Value("${orders.outbox.publisher.max-retries:3}") int maxRetries) {
    ...
    @Scheduled(fixedDelayString = "${orders.outbox.publisher.fixed-delay-ms:5000}")
    @Transactional
